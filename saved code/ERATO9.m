function GrowthRate = ERATO9(E,a,q0,n,n_r)
% change the method of calculate the non-orthogonality
...
fpsiGrad2Dpsi_rz = (fpsiGrad2Dr_rz.*fpsiDr_rz+fpsiGrad2Dz_rz.*fpsiDr_rz)...
    ./fpsiGradNorm_rz;
flogpsiGradNormDpsi_rz = fpsiGrad2Dpsi_rz./(2*fpsiGradNorm_rz.^2);
flogrDpsi_rz = 1./r.*fpsiDr_rz./fpsiGradNorm_rz;

fp_psi = @(psi) Psi_s*(1+E^2)*(Psi_s-psi)/(a^2*E^2*R^2);
fpDpsi_psi = @(psi) -Psi_s*(1+E^2)/(a^2*E^2*R^2)*ones(size(psi));
...
logpsiGradNormDpsi_rz = flogpsiGradNormDpsi_rz(cr,cz);
% the gradient psi direction's derivative of log r
logrDpsi_rz = flogrDpsi_rz(cr,cz);
...
for i = 2:n_s% exclude the magnetic axis situation
    ...
    path_left = fzero(@(r) fpsi_rz(r,0)-psi(i),[r_min*0.9,R]);
    path_right = fzero(@(r) fpsi_rz(r,0)-psi(i),[R,r_max*1.1]);
    
    % the initialize vector of rho
    rho_st(1,i) = path_right-R;
    rho_st(end,i) = R-path_left;
    for i_path = 2:(n_theta-1)% exclude the start and end point at which z = 0
        rho_st(i_path,i) = fzero(@(rho) ...
            fpsi_rz(R+rho*cos(ctheta(i_path)),...
            rho*sin(ctheta(i_path)))-psi(i),...
            [0,rho_max]);
    end
end
...
jphi_sc = interp2(cr,cz,jphi_rz,r_sc,z_sc);
psiGrad2Dpsi_sc = interp2(cr,cz,psiGrad2Dpsi_rz,r_sc,z_sc);

q_sc = interp1(s,q_s,cs);
qDpsi_sc = interp1(s,qDpsi_s,cs);
dchi = [zeros(1,n_s);cchi(2:end,:)-cchi(1:end-1,:)];
kernel = dchi.*(r_sc.*jphi_sc./(psiGradNorm_sc.^2)...
    -psiGrad2Dpsi_sc./(psiGradNorm_sc.^2)-qDpsi_sc./q_sc);

for i = 1:length(chi)
    betachi_sc(i,:) = sum(kernel(1:i,:));
end
...
pK = 2*ppsi/q0.*(pjphi.^2./ppsiGradNorm.^2+pjphi./pr.*plogpsiGradNormDpsi...
    -ppDpsi.*plogrDpsi);
...
c = ppsiGradNorm.^2.*pr.^2./(2*Psi_s*J).*acoeff;
d = pr.^4*gamma./(2*Psi_s*J).*acoeff;
e = pK.*2.*pr.^4*q0./J.*acoeff;
