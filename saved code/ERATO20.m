function GrowthRate = ERATO20(E,a,q0,n,n_s)
...
% n_newx = (3*n_s+1)*(2*n_chi+2);
% 
% i_X = (2*n_chi+2)*(3*(1:n_s+1)-3)+2*(2:n_chi-1)'-1;
% i_X = i_X(:);
% i_V = (2*n_chi+2)*(3*(1:n_s)-2)+2*(2:n_chi-1)'-1;
% i_V = i_V(:);
% i_Y = (2*n_chi+2)*(3*(1:n_s)-1)+2*(2:n_chi-1)'-1;
% i_Y = i_Y(:);
% % M = sparse(1:n_x,1:n_x,1,n_x,n_x)+...
% %     sparse(i_X  ,i_X  ,1,n_x,n_newx)+...%XR
% %     sparse(i_X  ,i_X+2,1,n_x,n_newx)+...
% %     sparse(i_X+1,i_X+1,1,n_x,n_newx)+...%XI
% %     sparse(i_X+1,i_X+3,1,n_x,n_newx)+...
% %     sparse(i_V  ,i_V  ,1,n_x,n_newx)+...%VR
% %     sparse(i_V  ,i_V+2,1,n_x,n_newx)+...
% %     sparse(i_V+1,i_V+1,1,n_x,n_newx)+...%VI
% %     sparse(i_V+1,i_V+3,1,n_x,n_newx)+...
% %     sparse(i_Y  ,i_Y  ,1,n_x,n_newx)+...%YR
% %     sparse(i_Y  ,i_Y+2,1,n_x,n_newx)+...
% %     sparse(i_Y+1,i_Y+1,1,n_x,n_newx)+...%YI
% %     sparse(i_Y+1,i_Y+3,1,n_x,n_newx);
% M = sparse(1:n_x,1:n_x,1,n_x,n_x)+...
%     sparse(i_X  ,i_X  ,1,n_x,n_newx)+...%XR
%     sparse(i_X  ,i_X+2,1,n_x,n_newx)+...
%     sparse(i_V+1,i_V+1,1,n_x,n_newx)+...%VI
...
% At = invM'*At*invM;
% Bt = invM'*Bt*invM;
% i_X = (2*n_chi+2)*(3*(1:n_s+1)-3)+2*[(1:n_chi-1),n_chi+1]'-1;
% i_X = i_X(:);
% i_XI = (2*n_chi+2)*(3*(1:n_s+1)-3)+2*(1:n_chi+1)'-1;
% i_XI = i_XI(:);
% i_V = (2*n_chi+2)*(3*(1:n_s)-2)+2*[(1:n_chi-1),n_chi+1]'-1;
% i_V = i_V(:);
% i_VR = (2*n_chi+2)*(3*(1:n_s)-2)+2*(1:n_chi+1)'-1;
% i_VR = i_VR(:);
% i_Y = (2*n_chi+2)*(3*(1:n_s)-1)+2*[(1:n_chi-1),n_chi+1]'-1;
% i_Y = i_Y(:);
% i_YR = (2*n_chi+2)*(3*(1:n_s)-1)+2*(1:n_chi+1)'-1;
% i_YR = i_YR(:);
% i_x = [i_X',i_XI'+1,i_VR',i_V'+1,i_YR',i_Y'+1];
% At = At(i_x,i_x);
% Bt = Bt(i_x,i_x);
