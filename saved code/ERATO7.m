function GrowthRate = ERATO7(Es,as,q0s,ns)

%change the chi range to [0,pi]
...
fpsi_rz = @(r,z) Psi_s/(a^2*R^2)*(r.^2.*z.^2/E^2+(r.^2-R^2).^2/4);
...
pchi = pi*(1/(2*n_chi):1/n_chi:1-1/(2*n_chi));
...
i_VI = (2*n_chi+2)*(3*(1:n_s)-2)+2;
i_YI = (2*n_chi+2)*(3*(1:n_s)-1)+2;
i_x = [i_XR,i_VI,i_YI];
i_x = [i_x,i_x+2*n_chi];
diagIndex = false(n_x,1);
diagIndex(i_x) = true;
% symmetry conditions
%tilda A
A(i_x,:) = 0;
...
w2 = eig(full(B\A));
mu0 = -1000;
[x,w2] = eigSolver(A,B,1e-8,mu0,rand(n_x,1));
w2 = w2+mu0;
disp(w2);
% figure(1);
% plot(w2);
