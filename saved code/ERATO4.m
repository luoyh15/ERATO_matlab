function GrowthRate = ERATO4(Es,as,q0s,ns)
%% set global variable
global R E a Psi_s
...
acoeff([1,end],:) = 1/2;
%% the matrix A
A =   I1R'*(a(:)./ps(:).*acoeff(:).*I1R) + I1I'*(a(:)./ps(:).*acoeff(:).*I1I)...
    + I2R'*(b(:)./ps(:).*acoeff(:).*I2R) + I2I'*(b(:)./ps(:).*acoeff(:).*I2I)...
    + I3R'*(c(:)./ps(:).*acoeff(:).*I3R) + I3I'*(c(:)./ps(:).*acoeff(:).*I3I)...
    + I4R'*(d(:)./ps(:).*acoeff(:).*I4R) + I4I'*(d(:)./ps(:).*acoeff(:).*I4I)...
    - I5R'*(e(:)./ps(:).*acoeff(:).*I5R) - I5I'*(e(:)./ps(:).*acoeff(:).*I5I);
%% the matrix B
B =  XR'*(f(:)./ps(:).*acoeff(:).*XR) + XI'*(f(:)./ps(:).*acoeff(:).*XI) +...
    (VR+YR-pbetachi(:).*XR)'*(g(:)./ps(:).*acoeff(:).*(VR+YR-pbetachi(:).*XR))+...
    (VI+YI-pbetachi(:).*XI)'*(g(:)./ps(:).*acoeff(:).*(VI+YI-pbetachi(:).*XI))+...
     YR'*(h(:)./ps(:).*acoeff(:).*YR) + YI'*(h(:)./ps(:).*acoeff(:).*YI);
% subplot(2,4,1);spy(A);
% subplot(2,4,5);spy(B);
% disp([rank(A),rank(B)]);
% A = sparse(A);
