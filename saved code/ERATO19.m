function GrowthRate = ERATO19(E,a,q0,n,n_s)
...
pchi = 1/(2*n_chi):pi/(n_chi):pi-pi/(2*n_chi);
% psi
ppsi = ps.^2*Psi_s;
% initialization
%safety factor
...
%     pr(end,i) = r_left;
    for j = 1:length(pchi)
        % r,z
        pr(j,i) = fzero(@(r) chi_r(r)-pchi(j),[r_left,R+(r_right-R)*cos(pi/(8*n_chi))]);
...
% symmetry conditions
i_X = (2*n_chi+2)*(3*(1:n_s+1)-3)+1;
i_V = (2*n_chi+2)*(3*(1:n_s)-2)+2;
i_Y = (2*n_chi+2)*(3*(1:n_s)-1)+2;
i_x = [i_X,i_X+2*n_chi,i_V,i_V+2*n_chi,i_Y,i_Y+2*n_chi];
%tilda A
At(i_x,:) = 0;
At(:,i_x) = 0;
