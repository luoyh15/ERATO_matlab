function GrowthRate = ERATO27(E,a,q0,n,n_s)
% non-equivalent mesh grid
...
%% generate non-equivalent s mesh by the nq = m singularity
% n_s = 14;
ms_eql = 0:1/n_s:1;
% psi
mpsi_eql = ms_eql.^2*Psi_s;
% initialization
%safety factor
mq_eql = zeros(size(mpsi_eql));
mq_eql(1) = q0;
for i = 2:length(ms_eql)
    % the start and end point of the loop integration
    r_left = double(subs(frleft_psi,psis,mpsi_eql(i)));
    r_right = double(subs(frright_psi,psis,mpsi_eql(i)));
    % the corresponding z
    z_path = subs(fz_rpsi,psis,mpsi_eql(i));
    % the quantites along the 'psi = constant' path
    T_path = subs(fT_rz,z,z_path);
    psiGradNorm_path = subs(fpsiGradNorm_rz,z,z_path);
    zDr_path = subs(fzDr_rpsi,psis,mpsi_eql(i));
...
%     pq(i) = double(1/pi*int(-fint_q,r_right,r_left));
    ...
    mq_eql(i) = 1/pi*integral(@(rr) kernel_q(rr),r_right,r_left);        
end
fq = fit(ms_eql',mq_eql','cubicinterp');
% fq = @(s) interp1(ms_eql,mq_eql,s,'spline');
m1 = ceil(n*q0); m2 = floor(n*mq_eql(end));
n_sing = 0;
x_sing = [];
for mi = m1:m2
    xi = fzero(@(ss) n*fq(ss)-mi,[0,1]);
    if ~isempty(xi)
        n_sing = n_sing+length(xi);
        x_sing = [x_sing,xi];
    end
end
delta_sing = 0.1./(n*differentiate(fq,x_sing))';
alpha_sing = 2*n_sing/(3*(n_sing+1))...
    /sum(atan((1-x_sing)./delta_sing)+atan(x_sing./delta_sing),2);
W_sing = @(s) (3+n_sing)/(3*(n_sing+1))*s+...
    alpha_sing*sum(atan((s-x_sing)./delta_sing)+atan(x_sing./delta_sing),2);
ms = ms_eql;
...
%     pqDpsi(i) = real(pqDpsi(i));
    ...
    %%%-------------------- Mercier's Criterion ------------------------%%%   
    % the coefficients
    kernel_C1 = pq(i)/T_path./psiGradNorm_path.^2.*fint_chi;
    kernel_C1 = matlabFunction(kernel_C1);
    C1 = 2*integral(@(rr) kernel_C1(rr),r_right,r_left);
    kernel_C2 = pq(i)/T_path*(r^2-R^2)^2./psiGradNorm_path.^2*fint_chi;
    kernel_C2 = matlabFunction(kernel_C2);
    C2 = 2*integral(@(rr) kernel_C2(rr),r_right,r_left);
...
% i_XR = (2*n_chi+2)*(3*(1:n_s+1)-3)+1;
% i_XI = i_XR+1;
% i_VR = (2*n_chi+2)*(3*(1:n_s)-2)+1;
% i_VI = i_VR+1;
% i_YR = (2*n_chi+2)*(3*(1:n_s)-1)+1;
% i_YI = i_YR+1;
