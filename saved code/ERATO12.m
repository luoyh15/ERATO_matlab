function GrowthRate = ERATO12(E,a,q0,n,n_r)
% change the directin of chi
...
logr2Ds_sc(:,1:end-1) = (2*log(r_sc(:,2:end))-2*log(r_sc(:,1:end-1)))...
    ./(cs(:,2:end)-cs(:,1:end-1));
logr2Ds_sc(:,end) = logr2Ds_sc(:,end-1);
logr2Dchi_sc(1:end-1,:) = (2*log(r_sc(2:end,:))-2*log(r_sc(1:end-1,:)))...
...
DVDchiI = sparse(i_cell,i_xij+1,DVDchiI_coef(1,:),n_cell,n_x)+...
    sparse(i_cell,i_xij+3,DVDchiI_coef(2,:),n_cell,n_x);
...
plogr2Ds = interp2(cs,cchi,logr2Ds_sc,ps,pchi,interpmethod);
plogr2Dchi = interp2(cs,cchi,logr2Dchi_sc,ps,pchi,interpmethod);
...
plogpsiGradNormDpsi = interp2(cr,cz,logpsiGradNormDpsi_rz,pr,pz,interpmethod);
% the gradient psi direction's derivative of log r
plogrDpsi = interp2(cr,cz,logrDpsi_rz,pr,pz,interpmethod);
...
I4R = plogr2Ds(:).*XR+plogr2Dchi(:).*(VR+YR)...
    -n*pq(:).*YI+DXDsR+DVDchiR+DYDchiR;
I4I = plogr2Ds(:).*XI+plogr2Dchi(:).*(VI+YI)...
