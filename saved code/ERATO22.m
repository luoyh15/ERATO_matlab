function GrowthRate = ERATO22(E,a,q0,n,n_s)
...
for j = 1:n_s
    for i = 1:n_chi
        ...
        if i==1
            sign = -1;
            conA(:,[3,7,12,16]) = conA(:,[3,7,12,16])+sign*conA(:,[1,5,10,14]);
            conA(:,[4,8,11,15]) = conA(:,[4,8,11,15])-sign*conA(:,[2,6,9,13]);
            conA([3,7,12,16],:) = conA([3,7,12,16],:)+sign*conA([1,5,10,14],:);
            conA([4,8,11,15],:) = conA([4,8,11,15],:)-sign*conA([2,6,9,13],:);
            conB(:,[3,7,12,16]) = conB(:,[3,7,12,16])+sign*conB(:,[1,5,10,14]);
            conB(:,[4,8,11,15]) = conB(:,[4,8,11,15])-sign*conB(:,[2,6,9,13]);
            conB([3,7,12,16],:) = conB([3,7,12,16],:)+sign*conB([1,5,10,14],:);
            conB([4,8,11,15],:) = conB([4,8,11,15],:)-sign*conB([2,6,9,13],:);
            conA = 1/2*conA;
            conB = 1/2*conB;
        ...
        if i==n_chi
            sign = -1;
            conA(:,[1,5,10,14]) = conA(:,[1,5,10,14])+sign*conA(:,[3,7,12,16]);
            conA(:,[2,6,9,13]) = conA(:,[2,6,9,13])-sign*conA(:,[4,8,11,15]);
            conA([1,5,10,14],:) = conA([1,5,10,14],:)+sign*conA([3,7,12,16],:);
            conA([2,6,9,13],:) = conA([2,6,9,13],:)-sign*conA([4,8,11,15],:);
            conB(:,[1,5,10,14]) = conB(:,[1,5,10,14])+sign*conB(:,[3,7,12,16]);
            conB(:,[2,6,9,13]) = conB(:,[2,6,9,13])-sign*conB(:,[4,8,11,15]);
            conB([1,5,10,14],:) = conB([1,5,10,14],:)+sign*conB([3,7,12,16],:);
            conB([2,6,9,13],:) = conB([2,6,9,13],:)-sign*conB([4,8,11,15],:);
            conA = 1/2*conA;
            conB = 1/2*conB;
        ...
        Map = Map(:);
        An(Map,Map) = An(Map,Map)+ds*dchi*conA;
        Bn(Map,Map) = Bn(Map,Map)+ds*dchi*conB;
    end
end
...
An(i_x,:) = 0;
An(:,i_x) = 0;
An = An + sparse(i_x,i_x,1,n_x,n_x);
% tilda B
Bn(i_x,:) = 0;
Bn(:,i_x) = 0;
Bn = Bn + sparse(i_x,i_x,w2inv,n_x,n_x);
...
[eigFunn,eigValn] = eig(full(An),full(Bn));
