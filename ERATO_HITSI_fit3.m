function ERATO_HITSI_fit3(filename)
...
%     fzmsk = double(subs(fz,r,R))>0&double(subs(fz,r,R))<=2*zmax_anl;
%     if fzmsk==0
%         pause(1);
%     end
%     z_path = fz(fzmsk);
...
for i = n_anl+1:10:n_s
    ...
    for j = 2:size(r_i,1)-1
        ...
        fitsave{j}.fpsi_rz = fpsi_rz;
        fitsave{j}.fr = fr;
        fitsave{j}.fz = fz;
        fitsave{j}.dr = dr;
        fitsave{j}.dz = dz;
        fitsave{j}.dl = dl;
        fitsave{j}.lleft = lleft;
        fitsave{j}.lright = lright;
        fitsave{j}.var = var;    
    end
    for j = 2:size(r_i,1)-1
        fr = fitsave{j}.fr;
        ...
        dz = fitsave{j}.dz;
        dl = fitsave{j}.dl;
        lleft = fitsave{j}.lleft;
        lright = fitsave{j}.lright;
        var = fitsave{j}.var;
        % psi as function of (r,z) and it's derivatives
        fpsi_rz = fitsave{j}.fpsi_rz;
        fpsiDr_rz =  diff(fpsi_rz,r);
        fpsiDz_rz =  diff(fpsi_rz,z);
        fpsiGradNorm_rz = sqrt(fpsiDr_rz.^2+fpsiDz_rz.^2);
        fpsiGrad2Dr_rz = diff(fpsiGradNorm_rz.^2,r);
        ...
        while ichi<n_chi&&pchi(ichi)<=chi_st(j,i)
            ...
            plnr2Dchi(ichi,i) = double(subs(2*pq(i)*fpsiGradNorm_rz/T_path...
                *dr/sqrt(dr^2+dz^2),{r,z},{pr(ichi,i),pz(ichi,i)}));
            % lnr2Ds
        ...
        pr(n_chi,i) = r_i(end,2);
        pz(n_chi,i) = z_i(end,2);
    end
end
pause(1);
