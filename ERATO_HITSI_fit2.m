function ERATO_HITSI_fit2(filename)
...
%     Msk_i = [false,false,data.psi(4:end)<ppsi(i)&data.psi(1:end-3)>ppsi(i)];
    ...
    for j = 1:size(r_i,2)-1
        ...
        fbound_up = polyfit(r_ij(1,:),z_ij(1,:),1);
        if abs(fbound_up(1))<1
            % z as a function of r 
            [fz,~,~] = solve(fpsi_rz-ppsi(i),z,'ReturnConditions',true);
            fzmsk = double(subs(fz,r,mean(r_ij(:))))>=min(min(z_ij))...
                &double(subs(fz,r,mean(r_ij(:))))<=max(max(z_ij));
            display(fzmsk);
            if fzmsk==false
                pause(1);
            end
            ...
            if r_ij(1,1)==r_ij(2,1)
                r_right = r_ij(1,1);
            else
                fbound_right = polyfit(r_ij(:,1),z_ij(:,1),1);
                fr_right = matlabFunction(fz-fbound_right*[r,1]');
                r1 = r_ij(Mpsi_ij(:,1)<=ppsi(i),1);
                r2 = r_ij(Mpsi_ij(:,1)>=ppsi(i),1);
                r_right = fzero(fr_right,[r1(1),r2(end)]);
            end
            % left boundary of the fit area
            if r_ij(1,end)==r_ij(2,end)
                r_left = r_ij(1,end);
            else
                fbound_left = polyfit(r_ij(:,end),z_ij(:,end),1);
                fr_left = matlabFunction(fz-fbound_left*[r,1]');
                r1 = r_ij(Mpsi_ij(:,end)<=ppsi(i),end);
                r2 = r_ij(Mpsi_ij(:,end)>=ppsi(i),end);
                r_left = fzero(fr_left,[r1(1),r2(end)]);
            end
            % plot the constant psi surface
            rr = linspace(r_left,r_right,10);
            figure(4);hold on;
            plot(rr,double(subs(fz,r,rr)));
        else
            % r as a function of z
            [fr,~,~] = solve(fpsi_rz-ppsi(i),r,'ReturnConditions',true);
            frmsk = double(subs(fr,z,mean(z_ij(:))))>=min(min(r_ij))...
                &double(subs(fr,z,mean(z_ij(:))))<=max(max(r_ij));
            display(frmsk);
            if frmsk==false
                pause(1);
            end
            fr = fr(frmsk);
            % right boundary of the fit area
            if z_ij(1,1)==z_ij(2,1)
                z_right = z_ij(1,1);
            else
                fbound_right = polyfit(z_ij(:,1),r_ij(:,1),1);
                fz_right = matlabFunction(fr-fbound_right*[z,1]');
                z1 = z_ij(Mpsi_ij(:,1)<=ppsi(i),1);
                z2 = z_ij(Mpsi_ij(:,1)>=ppsi(i),1);
                z_right = fzero(fz_right,[z1(1),z2(end)]);
            end
            % left boundary of the fit area
            if z_ij(1,end)==z_ij(2,end)
                z_left = z_ij(1,end);
            else
                fbound_left = polyfit(z_ij(:,end),r_ij(:,end),1);
                fz_left = matlabFunction(fr-fbound_left*[z,1]');
                z1 = z_ij(Mpsi_ij(:,end)<=ppsi(i),end);
                z2 = z_ij(Mpsi_ij(:,end)>=ppsi(i),end);
                z_left = fzero(fz_left,[z1(1),z2(end)]);
            end
            % plot the constant psi surface
            zz = linspace(z_left,z_right,10);
            figure(4);hold on;
            plot(double(subs(fr,z,zz)),zz);
        end
